#!/bin/bash
# Sign the app with an identity that stays the same.
#
# Wails only signs ad-hoc by itself. The code hash then changes on every build,
# macOS takes the app for a new one and asks for the permissions again — on
# every single start. With a fixed self-signed certificate and a stable bundle
# id the system remembers the decision.
set -e
...
EOF
	...
	# macOS cannot read the newer pkcs12 defaults of openssl
